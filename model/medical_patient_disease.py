...
class medical_patient_disease(models.Model):
    ...
    patient_disease_count = fields.Integer(compute='patient_disease_compute_count')

    def patient_disease_compute_count(self):
        for record in self:
            record.patient_disease_count = self.env['"medical.patient'].search_count(
                [('patient_disease_ids', '=', self.id)])

    def get_patient_disease(self):
        return {
            'name': _('patient disease'),
            'view_type':'form',
